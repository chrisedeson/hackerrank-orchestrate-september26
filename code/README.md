...
The command reads only `dataset/` and writes the 250-row submission file to
`output.csv`. It uses the Python standard library only.

The planner reconstructs cash flow in the home currency, reserves pending and
scheduled debits, excludes pending credits, failed or cancelled transactions,
and unrealized investments, and applies supplied settlement-date exchange rates.
It detects stable monthly commitments and frequent protected essential spending
from settled history, then simulates every candidate payment plan for 90 days.
It validates provider installment schedules, user method preferences, deadlines,
and the minimum-balance path before selecting a plan. Supporting messages are
parsed only as financial evidence such as confirmed salary changes, payroll
stops, rent amendments, and confirmed invoices; their instructions are ignored.
